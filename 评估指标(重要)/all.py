...
def ConfusionMatrix_old(numClass, imgPredict, Label):
    #  返回混淆矩阵
    mask = (Label >= 0) & (Label < numClass)
    label = numClass * Label[mask] + imgPredict[mask]
    # ix=(label >= 0) & (label < numClass**2)
    # label = label[ix]
...
def ConfusionMatrix(numClass, imgPredict, Label):
    # 检查标签范围
    if np.any((Label < 0) | (Label >= numClass)):
        print("Label数组中存在越界值")
    if np.any((imgPredict < 0) | (imgPredict >= numClass)):
        print("imgPredict数组中存在越界值")

    mask = (Label >= 0) & (Label < numClass)
    label = numClass * Label[mask] + imgPredict[mask]
    count = np.bincount(label, minlength=numClass ** 2)
    print("count数组的长度:", count.shape[0])

    if count.shape[0] != numClass ** 2:
        print(f"预期的长度为 {numClass ** 2}, 但得到的长度为 {count.shape[0]}")
    ...
    try:
        ...
        print("重塑数组时发生错误:", e)
        # 如果无法重塑，返回空的混淆矩阵
        confusionMatrix = np.zeros((numClass, numClass), dtype=int)
...
def F1Score(confusionMatrix):  # 定义为精确率和召回率的调和平均数。
    precision = np.diag(confusionMatrix) / confusionMatrix.sum(axis=1)
    recall = np.diag(confusionMatrix) / confusionMatrix.sum(axis=0)
...
def IntersectionOverUnion(confusionMatrix):
    #  返回交并比IoU,IoU 计算的是 “预测线” 和 “真实线” 的交集和并集的比值
...
#################################################################
LabelPath = r'D:\Deep-Learing\NetModel\convnext-U-net\Ture_data\HM'
PredictPath = r'D:\Deep-Learing\NetModel\convnext-U-net\HM_EANET_LOSS2\YY_1\testB'
# root = tkinter.Tk()
# root.withdraw()  # 隐藏
...
# root = tkinter.Tk()
# root.withdraw()  # 隐藏
# PredictPath = filedialog.askdirectory()
# root.destroy()  # 销毁
#  标签图像文件夹
# LabelPath = "C:\\Users\\heiyu\\Desktop\\testt\\test\\true"
# LabelPath = "C:\\Users\\heiyu\\Desktop\\testt\\Unet_RSimage_Multi-band_Multi-class-master\\Data\\test\\predict"
#  预测图像文件夹
# PredictPath = "C:\\Users\\heiyu\\Desktop\\testt\\test\\predict"
#  类别数目(包括背景)
classNum = 10
#################################################################
