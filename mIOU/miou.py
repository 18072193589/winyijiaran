...
def keep_image_size_open_label(path, size=(1024, 1024)):
    img = Image.open(path)
    temp = max(img.size)
...
def keep_image_size_open_predict(path, size=(1024, 1024)):
    img = Image.open(path)
    temp = max(img.size)
    mask = Image.new('P', (temp, temp))
    mask.paste(img, (0, 0))
...
if __name__ == '__main__':
    ...
    label_path = r"D:\Deep-Learing\NetModel\util\mIOU\rtvue_to_canon\lable" # 标签的文件夹位置

    predict_path = r"D:\Deep-Learing\NetModel\util\mIOU\rtvue_to_canon\pred" # 预测结果的文件夹位置
