...
# 设置图像文件夹路径
folder_path = r'D:\Deep-Learing\NetModel\util\计算各层厚度（重要）\画分界线\HM'
...
for filename in os.listdir(folder_path):
    if filename.endswith('.png'):
        ...
        # 高斯模糊，减少噪声，平滑图像
        blurred_image = cv2.GaussianBlur(segmentation_image, (5, 5), 0)
        ...
        for value, color in colors.items():
            # 找到边界
            binary_mask = (segmentation_image == value).astype(np.uint8) * 255
